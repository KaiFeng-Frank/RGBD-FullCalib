...
v0.1 交付第一个完整实例:一台 D435i 从头标到尾。[路线图](ROADMAP.md)走得远得多。**
...
模型 → cam-IMU → 加速度计内参 → 温漂模型),全套工具、判决层、GUI、15+ 条真实
时间换来的坑。

下一步的接缝已经在代码里:约一半工具本就与设备无关(英文 README 目录表里逐个
标注了)、viewer 的源抽象层为「不是相机的传感器」而设计(协议里给在路上的 Mid-360
留了原生点云通道)、v0.2 把判决规则声明式化 —— 加设备变成写规则,不是 fork 代码。

### 拿你的相机来撞

在别的 RGB-D / 视惯 rig 上跑任一阶段撞了墙,把 traceback 开成 issue ——
v0.2+ 先抽象什么,由真实断点决定,不靠猜。有一个常驻 issue 就收这个。
...
**v0.2** 判决层数据化(声明式规则) · **v0.3** Mid-360 进场,雷视-IMU 跨传感器全链 ·
